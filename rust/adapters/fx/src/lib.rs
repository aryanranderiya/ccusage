...
use crate::{
    Result, cli::AgentCommandArgs, print_json_or_jq, print_usage_table, sort_summaries,
    wants_json,
};
...
pub fn run(args: AgentCommandArgs) -> Result<()> {
    ...
    let mut entries = load_entries(&args.shared, args.fx_path.as_deref(), Some(&pricing))?;
